...
public class TicketSQLiteOpenHelper extends SQLiteOpenHelper {
	
	private static final String DB_NAME = "tickets_lock.sqlite3";
	...
	private static TicketSQLiteOpenHelper instance;
	
	public static TicketSQLiteOpenHelper getInstance(Context context) {
	    if (null == instance) {
	        instance = new TicketSQLiteOpenHelper(context);
	    }
	    return instance;
	}
	
	public TicketSQLiteOpenHelper(Context context) {
...
//		database.update(TICKETS_TABLE, values, where, null);
	...
	public int countickets() {
        Cursor ticketsCursor = getReadableDatabase().query(TICKETS_TABLE, new String[] {
                TICKETS_ID, 
                TICKETS_NAME, 
                TICKETS_COMPLETE}, 
                null, null, null, null, String.format("%s, %s", TICKETS_COMPLETE, TICKETS_NAME));
        int amount = ticketsCursor.getCount();
        ticketsCursor.close();
        return amount;
    }	
}
