...
package com.cornucopia.widgets.material.edit;
...
import com.cornucopia.widgets.material.edit.validation.METValidator;
...
public class MEditText extends EditText {
    ...
    @IntDef({
            FLOATING_LABEL_NONE, FLOATING_LABEL_NORMAL, FLOATING_LABEL_HIGHLIGHT
    })
    ...
    /**
     * the spacing between the main text and the inner components (floating
     * label, bottom ellipsis, characters counter).
     */
    ...
    /**
     * characters count limit. 0 means no limit. default is 0. NOTE: the
    ...
    @SuppressLint("NewApi")
    public MEditText(Context context, AttributeSet attrs, int style) {
        ...
        innerComponentsSpacing = getResources().getDimensionPixelSize(
        ...
        baseColor = typedArray.getColor(R.styleable.MEditText_baseColor, Color.BLACK);
        ColorStateList colorStateList = new ColorStateList(new int[][] {
                new int[] {
                    android.R.attr.state_enabled
                }, EMPTY_STATE_SET
        }, new int[] {
                baseColor & 0x00ffffff | 0xdf000000, baseColor & 0x00ffffff | 0x44000000
        });
        setTextColor(colorStateList);

        primaryColor = typedArray.getColor(R.styleable.MEditText_primaryColor, baseColor);
        setFloatingLabelInternal(typedArray.getInt(R.styleable.MEditText_floatingLabel, 0));
        errorColor = typedArray.getColor(R.styleable.MEditText_errorColor,
                Color.parseColor("#e7492E"));
        maxCharacters = typedArray.getInt(R.styleable.MEditText_maxCharacters, 0);
        singleLineEllipsis = typedArray.getBoolean(R.styleable.MEditText_singleLineEllipsis,
                false);
        helperText = typedArray.getString(R.styleable.MEditText_helperText);
        minBottomTextLines = typedArray.getInt(R.styleable.MEditText_minBottomTextLines, 0);
        String fontPath = typedArray.getString(R.styleable.MEditText_accentTypeface);
        if (fontPath != null) {
            accentTypeface = getCustomTypeface(fontPath);
            textPaint.setTypeface(accentTypeface);
        }
        floatingLabelText = typedArray.getString(R.styleable.MEditText_floatingLabelText);
        if (floatingLabelText == null) {
            floatingLabelText = getHint();
        }
        hideUnderline = typedArray.getBoolean(R.styleable.MEditText_hideUnderline, false);
        typedArray.recycle();
    ...
    private Typeface getCustomTypeface(@NonNull
    String fontPath) {
     ...
     */
    public void setFloatingLabelText(@Nullable
    ...
    private int getPixel(int dp) {
        return MaterialUtils.dpToPx(dp, getResources());
    }

    private void initPadding() {
        int paddingTop = getPaddingTop() - extraPaddingTop;
        int paddingBottom = getPaddingBottom() - extraPaddingBottom;
        extraPaddingTop = floatingLabelEnabled ? floatingLabelTextSize + innerComponentsSpacing
                : innerComponentsSpacing;
        extraPaddingBottom = (int) ((fontMetrics.descent - fontMetrics.ascent) * currentBottomLines)
                + (hideUnderline ? innerComponentsSpacing : innerComponentsSpacing * 2);
    ...
    private void initMinBottomLines() {
        boolean extendBottom = maxCharacters > 0 || singleLineEllipsis || tempErrorText != null
                || helperText != null;
    ...
    public void setFloatingLabel(@FloatingLabelType
    int mode) {
    ...
    private void adjustBottomLines() {
        ...
        if (tempErrorText != null) {
            textLayout = new StaticLayout(tempErrorText, textPaint, getWidth()
                    - getBottomTextLeftOffset() - getBottomTextRightOffset(),
                    Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
            destBottomLines = Math.max(textLayout.getLineCount(), minBottomTextLines);
        } else if (helperText != null) {
            textLayout = new StaticLayout(helperText, textPaint, getWidth()
                    - getBottomTextLeftOffset() - getBottomTextRightOffset(),
                    Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
            destBottomLines = Math.max(textLayout.getLineCount(), minBottomTextLines);
        } else {
     ...
     */
    public boolean validateWith(@NonNull
    METValidator validator) {
        CharSequence text = getText();
    ...
    /**
     * Check all validators, sets the error text if not NOTE: this stops at the
     * first validator to report invalid.
     *
    ...
    /**
     * Adds a new validator to the View's list of validators This will be
     * checked with the others in {@link #validate()}
     *
     * @param validator Validator to add
     * @return This instance, for easy chaining
     */
    public MEditText addValidator(METValidator validator) {
    ...
    Canvas canvas) {
        ...
        if (!hideUnderline) {
            ...
            if (!isInternalValid()) { // not valid
                ...
                canvas.drawRect(getScrollX(), lineStartY, getWidth() + getScrollX(), lineStartY
                        + getPixel(2), paint);
            } else if (!isEnabled()) { // disabled
                paint.setColor(baseColor & 0x00ffffff | 0x44000000);
                float interval = getPixel(1);
                for (float startX = 0; startX < getWidth(); startX += interval * 3) {
                    canvas.drawRect(getScrollX() + startX, lineStartY, getScrollX() + startX
                            + interval, lineStartY + getPixel(1), paint);
                }
        ...
        if ((hasFocus() && maxCharacters > 0) || !isMaxCharactersValid()) {
            ...
            canvas.drawText(text, getWidth() + getScrollX() - textPaint.measureText(text),
                    lineStartY + innerComponentsSpacing + relativeHeight, textPaint);
        }
        ...
        if (floatingLabelEnabled && !TextUtils.isEmpty(floatingLabelText)) {
            ...
            // draw the floating label
            canvas.drawText(floatingLabelText.toString(), getPaddingLeft() + getScrollX(),
                    position, textPaint);
        }
        ...
        if (hasFocus() && singleLineEllipsis && getScrollX() != 0) {
            ...
            canvas.drawCircle(bottomEllipsisSize / 2 + getScrollX(), startY + bottomEllipsisSize
                    / 2, bottomEllipsisSize / 2, paint);
